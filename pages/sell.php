        ...
        if ($listing->insert()) {
            ...
            if (!empty($_FILES['photos']['name'][0])) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $filename = uniqid("listing_{$listingId}_", true) . '.' . strtolower($ext);
                    $dest     = $uploadDir . $filename;

                    if (move_uploaded_file($files['tmp_name'][$i], $dest)) {
                        $listing->addPhoto($listingId, '/uploads/listings/' . $filename, $i);
                    }
                }
            }
...
<style>
  ...
  .form-error {
    ...
    color: #c0392b; padding: 12px 16px; border-radius: 10px;
...
<nav id="top-nav">
  ...
  <a href="sell.php"><button class="btn-sell active">+ Sell</button></a>
</nav>
...
<main id="app">
  <div class="sell-page">
    ...
    <form method="POST" action="sell.php" enctype="multipart/form-data" id="sell-form" novalidate>
      ...
      <div class="upload-zone" id="upload-zone">
        ...
        <div class="upload-hint">Tap to upload or drag & drop</div>
      </div>
      ...
      <div class="form-group">
        ...
        <input type="text" id="f-title" name="name" class="form-input"
               placeholder="e.g. Vintage Levi's 501 Jeans W32" maxlength="150"
...
<nav id="bottom-nav">
  ...
  <a class="bottom-item sell-btn active" href="sell.php">
...
<script>
// ── Photo preview (client-side only, actual upload via form) ──
let selectedFiles = [];
...
function addFiles(files) {
  ...
  selectedFiles = [...selectedFiles, ...imgs.slice(0, remaining)];
  syncInput();
  renderPreviews();
}
...
// Keep the real file input in sync so the form submits the right files
function syncInput() {
...
document.getElementById('sell-form').addEventListener('submit', e => {
  ...
  if (!title) {
    ...
    errT.style.display = 'block'; ok = false;
  } else {
  ...
  if (!price || price <= 0) {
    document.getElementById('f-price').classList.add('error');
    errP.style.display = 'block'; ok = false;
  } else {
    document.getElementById('f-price').classList.remove('error');
  ...
  if (!cond || cond === '0') {
    errC.style.display = 'block'; ok = false;
  } else {
...
document.getElementById('search-input')?.addEventListener('keydown', e => {
  if (e.key === 'Enter')
    window.location.href = `explore.php?q=${encodeURIComponent(e.target.value)}`;
});
</script>
</body>
</html>