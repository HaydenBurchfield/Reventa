...
$stmt = $dbConn->prepare("SELECT COUNT(*) as cnt FROM listing_like WHERE listing_id=?");
$stmt->bind_param("i", $listingId);
$stmt->execute();
$likeCount = (int)$stmt->get_result()->fetch_assoc()['cnt'];
$stmt->close();
if ($isLoggedIn) {
    $stmt = $dbConn->prepare("SELECT 1 FROM listing_like WHERE user_id=? AND listing_id=?");
    $stmt->bind_param("ii", $currentUserId, $listingId);
    $stmt->execute();
...
<style>
  ...
  .badge { padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; background: #f0f0f0; color: #555; }
  ...
  .seller-card { display: flex; align-items: center; gap: 12px; background: #f8f8f8; border-radius: 14px; padding: 14px 16px; margin: 0 16px 16px; text-decoration: none; color: inherit; transition: background .15s; }
  ...
  .btn-buy { width: 100%; padding: 15px; background: #111; color: #fff; border: none; border-radius: 12px; font-size: 16px; font-weight: 700; cursor: pointer; font-family: inherit; transition: opacity .15s; }
  .btn-buy:hover { opacity: .85; } .btn-buy:disabled { opacity: .4; cursor: not-allowed; }
  .btn-msg { width: 100%; padding: 13px; background: #fff; color: #111; border: 1.5px solid #ddd; border-radius: 12px; font-size: 15px; font-weight: 600; cursor: pointer; font-family: inherit; transition: all .15s; }
  .btn-msg:hover { border-color: #111; }
  .btn-seller-action { width: 100%; padding: 13px; background: #fff; color: #e53935; border: 1.5px solid #e53935; border-radius: 12px; font-size: 15px; font-weight: 600; cursor: pointer; font-family: inherit; }

  /* Like button */
  .btn-like {
    width: 100%; padding: 13px; background: #fff; color: #555;
  ...
  .btn-like.liked { border-color: #e53935; color: #e53935; background: #fff5f5; }
  @keyframes heartPop { 0%{transform:scale(1)} 40%{transform:scale(1.5)} 100%{transform:scale(1)} }
  .heart-icon { transition: transform .1s; }
  .btn-like.pop .heart-icon { animation: heartPop .35s ease forwards; }
  ...
  .modal-sheet { background: #fff; border-radius: 20px 20px 0 0; padding: 24px 20px 40px; width: 100%; max-width: 680px; animation: slideUp .2s ease; }
  @keyframes slideUp { from{transform:translateY(100%)} to{transform:translateY(0)} }
  .modal-sheet h3 { margin: 0 0 14px; font-size: 18px; font-weight: 700; }
  .modal-sheet textarea { width: 100%; min-height: 90px; padding: 12px; border: 1.5px solid #e0e0e0; border-radius: 10px; font-family: inherit; font-size: 14px; resize: none; box-sizing: border-box; outline: none; }
  .modal-sheet textarea:focus { border-color: #111; }
  .modal-actions { display: flex; gap: 10px; margin-top: 12px; }
  .modal-send { flex: 1; padding: 13px; background: #111; color: #fff; border: none; border-radius: 10px; font-size: 15px; font-weight: 700; cursor: pointer; font-family: inherit; }
  .modal-cancel { padding: 13px 20px; background: #fff; border: 1.5px solid #ddd; border-radius: 10px; font-size: 14px; cursor: pointer; font-family: inherit; }
  .alert-banner { margin: 12px 16px; padding: 12px 16px; border-radius: 10px; font-size: 14px; font-weight: 500; }
  ...
  <div class="listing-page">
    ...
    <div class="listing-info">
      <div class="listing-badges">
        <?php if ($item['is_sold']): ?><span class="badge badge-sold">SOLD</span><?php endif; ?>
        <?php if (!empty($item['condition_name'])): ?><span class="badge"><?= htmlspecialchars($item['condition_name']) ?></span><?php endif; ?>
        <?php if (!empty($item['category_name'])): ?><span class="badge"><?= htmlspecialchars($item['category_name']) ?></span><?php endif; ?>
      </div>
    ...
    <a class="seller-card" href="#">
      <img class="seller-avatar" src="<?= sellerAvatar($item) ?>" alt="<?= htmlspecialchars($item['seller_username']) ?>">
      ...
      <?php else: ?>
        ...
        <button class="btn-like <?= $isLiked ? 'liked' : '' ?>"
          ...
          <svg class="heart-icon" width="18" height="18" viewBox="0 0 24 24"
               ...
               stroke="<?= $isLiked ? '#e53935' : 'currentColor' ?>"
               stroke-width="2">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
          </svg>
          <span id="like-label"><?= $isLiked ? 'Saved' : 'Save' ?></span>
...
<div class="modal-overlay" id="msg-modal">
  <div class="modal-sheet">
    ...
    <form method="POST">
      ...
      <textarea name="initial_message" placeholder="Hi, is this still available? …"></textarea>
...
<div class="modal-overlay" id="buy-modal">
  <div class="modal-sheet">
    ...
    <p style="font-size:14px;color:#555;margin:0 0 16px">Send the seller a message to arrange payment and pickup/shipping.</p>
    <form method="POST">
      <input type="hidden" name="action" value="message">
      <textarea name="initial_message">Hi! I'd like to buy this. Is it still available?</textarea>
      <div class="modal-actions">
...
galleryEl?.addEventListener('touchstart', e => { touchX = e.touches[0].clientX; });
...
document.getElementById('btn-msg')?.addEventListener('click', () => msgModal?.classList.add('open'));
document.getElementById('modal-cancel')?.addEventListener('click', () => msgModal?.classList.remove('open'));
document.getElementById('btn-buy')?.addEventListener('click', () => buyModal?.classList.add('open'));
document.getElementById('buy-cancel')?.addEventListener('click', () => buyModal?.classList.remove('open'));
[msgModal, buyModal].forEach(m => m?.addEventListener('click', e => { if (e.target===m) m.classList.remove('open'); }));
...
document.getElementById('btn-like')?.addEventListener('click', async function() {
  ...
  svg.setAttribute('fill',   data.liked ? '#e53935' : 'none');
  svg.setAttribute('stroke', data.liked ? '#e53935' : 'currentColor');
  document.getElementById('like-label').textContent = data.liked ? 'Saved' : 'Save';
  document.getElementById('like-count').innerHTML   = data.count > 0 ? '&nbsp;· ' + data.count : '';
});
</script>
</body>
</html>